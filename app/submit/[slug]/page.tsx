...
export default async function SubmitPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: inbox } = await supabase.from("inboxes").select("*").eq("slug", slug).eq("is_active", true).single()
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          <Card className="border-border/50 max-w-md shadow-lg">
            <CardHeader className="text-center space-y-3">
              ...
              <CardDescription>
                This Inbox is temporarily not accepting submissions. Please check back later.
              </CardDescription>
            </CardHeader>
          </Card>
        </main>
